	...
	<el-dialog v-model="dialogVisible" :title="currentUpdateId === undefined ? '新增' : '修改'" @close="resetForm" width="50%">
		<el-form ref="formRef" :model="formData" :rules="formRules" label-width="100px" label-position="right">
			...
			<el-form-item prop="publishDate" label="发布时间">
				<el-input v-model="formData.publishDate" placeholder="请输入" />
			</el-form-item>
...
import { getApi, createApi, updateApi } from "@/api/blogs/comment"
	...
	if (id === undefined) {
		...
		getApi({
			...
			.then((res: any) => {
				...
				formData.publishDate = res.data.publishDate
...
const formRules: FormRules = reactive({
	content: [{ required: true, trigger: "blur", message: "请输入名称" }],
